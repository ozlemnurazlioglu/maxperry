<div class="admin-dashboard">
    ...
    <main class="admin-content" style="max-width: 600px;">
        <div class="admin-header-row">
            <h1 style="font-size: 28px; color: var(--text-dark);">Kategoriyi Düzenle</h1>
        ...
        <div style="background-color: var(--bg-white); border: 1px solid var(--border-color); padding: 35px; box-shadow: var(--shadow-soft);">
            <form action="<?php echo BASE_URL; ?>/admin/category/edit/<?php echo $category['id']; ?>/submit" method="POST">
                ...
                <div class="form-group">
                    ...
                    <input type="text" id="name" name="name" class="form-control" value="<?php echo htmlspecialchars($category['name']); ?>" style="background-color: var(--bg-white);" required>
                </div>

                <!-- Description -->
                <div class="form-group" style="margin-bottom: 30px;">
                    <label for="description">Kategori Açıklaması</label>
                    <textarea id="description" name="description" rows="4" class="form-control" style="resize: none; background-color: var(--bg-white);"><?php echo htmlspecialchars($category['description'] ?? ''); ?></textarea>
                </div>

                <!-- Update Button -->
                <button type="submit" class="btn btn-rose" style="width: 100%; padding: 15px; font-weight: bold; letter-spacing: 1px;">
                    KATEGORİYİ GÜNCELLE <i class="fa-solid fa-circle-check" style="margin-left: 5px;"></i>
                </button>
            </form>
        </div>
    </main>
